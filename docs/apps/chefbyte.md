...
- **Undo (unmark done):** Completed meals can be undone via `private.unmark_meal_done`. This reverses the operation: deletes the food_logs tagged with the meal_id, restores consumed stock to the user's default location, and for meal prep entries deletes the `[MEAL]` product and its stock lot. The meal's `completed_at` is set back to NULL. **Product identity is id-based** (migration `20260515100000`): `mark_meal_done` stores the created `[MEAL]` product's id on `meal_plan_entries.meal_product_id`, and `unmark_meal_done` deletes that product **by id** — so undoing the 2nd meal-prep of the same recipe on the same date (whose product name carries a disambiguating time suffix) removes the correct product instead of the 1st meal's bare-named one (H-10). Meal-prep entries completed before that migration have `meal_product_id = NULL` and fall back to best-effort bare-name matching. Food_logs are tagged with `meal_id` during `mark_meal_done` by capturing the exact `food_log_id` each `consume_product` call returns (not by `created_at`), so unrelated same-transaction logs are never swept into the meal (H-14).
