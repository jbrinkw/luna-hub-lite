...
| Device Auth           | LiveTrack provisioning validates a one-time device key (stored hashed in `chefbyte.live_shelf_devices.import_key_hash`). Runtime events are accepted by x-api-key header hashed at request time, with JWT verification disabled on the IoT endpoints (`shelf-ingest`, `livetrack-session /pi-update`) |
