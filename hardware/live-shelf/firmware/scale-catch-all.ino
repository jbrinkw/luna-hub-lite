// ============================================================================
// scale-catch-all.ino — Catch-all scale firmware
//
// Identical to scale-live.ino except for the constants below:
//   - NUM_CELLS:   4 → 1    (single HX711 under a countertop platform)
//   - DOUT_PINS:   { D6, D1, D2, D5 } → { D6 }
//   - device_id:   "scale-01" → "scale-02"
// Same protocol, same stability state machine, same OTA + retry FIFO +
// onboard calibration web UI + LED. Everything else is a verbatim copy.
//
// If the catch-all has no WS2812B wired, D4 writes are harmless (floating
...
//
// Choice: the shelf weight used for event detection is the SUM of the 4
// cells. Legacy scale-test.ino calibration prompts for known weight on ONE
// cell at a time, so each cell's scale_factor maps its own raw reading to
// its own share of force in grams. Distributed load: each cell reports its
// portion, sum = true total. See README.md §"Weight definition".
//
...
//
// IMPORTANT — MAGIC differs from scale-live.ino (0xBEEF0005UL) on purpose:
// `CalStorage` contains `CellCal cells[NUM_CELLS]`, and NUM_CELLS on this
// firmware is 1 vs 4 on scale-live. Same magic+version with a different
// struct size means the magic/version check would pass on a Wemos that
// previously ran scale-live, and `RuntimeCfg` would then be read from the
// wrong byte offset (garbage device_id / pi_url). Bumping the magic forces
// `loadFromEEPROM()` to fail the check on cross-flashed hardware and
// re-initialize cleanly. EEPROM_VERSION stays at 3 — layout semantics
// within this firmware are unchanged.
#define EEPROM_SIZE 512
#define EEPROM_MAGIC 0xBEEF0006UL
...
struct RuntimeCfg {
  ...
  char pi_url[64];
  // Device id sent in payloads (default "scale-02" on the catch-all).
  char device_id[24];
...
// hits the HX711 in a transiently busy state. See handleTare comment.
...
bool readShelfWeight(float* out_weight) {
  ...
  // Cache for emergency-tare fallback. See handleTare comment.
...
// `out` on success. Falls back to scaleLoop's cached raw reading when
// fresh reads fail, marking `*used_cached`. Tare is the only recovery path
// from a drifted offset, so it must succeed whenever scaleLoop is reading.
bool captureTareRaw(long out[NUM_CELLS], bool* used_cached) {
  *used_cached = false;
  // Single-read retry. /data uses readRaw() once and works reliably; the
  // 10-sample readAverage path multiplied per-sample timeouts and could
  // fail tare even when the scale was reading fine in heartbeats.
