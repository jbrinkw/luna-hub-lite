-- mark_meal_done_distinct_default: [MEAL] products get is_distinct_unit_item=true,
-- default_recipe_unit='serving' so they are treated as discrete servings in recipes.
--
-- Uses pg_get_functiondef splice pattern — minimal diff over the previous version
-- (mark_meal_done_gram_unit, 20260429230000). Both INSERT INTO chefbyte.products
-- blocks (recipe-based and product-based) gain the two new columns.
