-- In-flight tracking on stock lots (cloud-side).
--
-- Adds `in_flight_since` and `pickup_event_id` to chefbyte.stock_lots so the
-- Inventory UI can render a small "In-flight" badge when a lot has been
-- picked up off a live scale / shelf but hasn't been reconciled back.
--
-- DESIGN NOTE (Phase 1 choice, see commit body):
--   The cloud's `chefbyte.shelf_event_log` only stores *terminal* event kinds
--   (consumed / depleted / added / refilled). The Pi's transient
--   `in_flight_pickup` state maps to `None` in PATTERN_TO_EVENT_KIND — it is
--   NEVER posted to cloud. So there is no `remove`-without-subsequent-`add`
--   row to derive in-flight state from at query time.
--
--   Therefore the only cloud-native path is an explicit column on stock_lots.
--   The column starts NULL for all existing rows (backfill = no-op) and the
--   badge simply won't render until something sets it. A future Pi change
--   will flip it via a new RPC; until then the UI surface is ready and the
--   behaviour matches "no in-flight lots" for every user.
--
--   apply_shelf_event is extended so that `added`/`refilled` events on a lot
--   automatically CLEAR `in_flight_since` — a normal "put back on the shelf"
--   flow thus closes the in-flight window without needing a dedicated RPC.
...
ALTER TABLE chefbyte.stock_lots
  ADD COLUMN IF NOT EXISTS in_flight_since  TIMESTAMPTZ NULL,
  ADD COLUMN IF NOT EXISTS pickup_event_id  UUID NULL;

-- Partial index — lets the UI (or a future sweeper) cheaply find every
-- in-flight lot for a user without scanning the full table. Matches the
-- same per-user pattern used on other stock_lots indexes.
CREATE INDEX IF NOT EXISTS stock_lots_in_flight_idx
  ON chefbyte.stock_lots (user_id, in_flight_since)
  WHERE in_flight_since IS NOT NULL;

------------------------------------------------------------
-- 2. Extend apply_shelf_event to clear in_flight_since on add/refill
------------------------------------------------------------
-- Full CREATE OR REPLACE of the 10-arg signature introduced in the
-- event_overrides migration. Only the `added`/`refilled` UPDATE changes:
-- it now also sets `in_flight_since = NULL` and `pickup_event_id = NULL`
-- so the "put it back on the shelf" flow closes the badge automatically.
