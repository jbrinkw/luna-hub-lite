...
/**
 * invariant-monitor — runs every 30 min; checks 10 system invariants
 ...
 *
 * Coverage scope: this monitor catches the 10 invariants below + any
...
const INVARIANTS: InvariantSpec[] = [
  ...
  },
  {
    ...
    intent:
      'For every (user, logical_date) the SUM(food_logs.calories) should be reproducible from the underlying consume events (4 * carbs + 4 * protein + 9 * fat) within ±10 kcal — drift > 10 kcal indicates either the macro-recompute trigger is broken or a manual log was inserted with bad math.',
    check: async (sb) => {
  ...
  {
    name: 'stock_lots_lifecycle_invariant',
    severity: 'error',
    subject_type: 'stock_lot',
    intent:
      'A lot with in_flight_since older than 24h (well past the 6h TTL reaper window) AND no matching pickup_event_id row in shelf_event_log is orphaned — the reaper should have either cleared the marker or removed the row.',
    check: async (sb) => {
  ...
  {
    name: 'pi_cloud_lot_id_consistency',
    severity: 'warning',
    subject_type: 'pi_cloud_pair',
    intent:
      'Pi-side check that every cloud stock_lots row touched in the last 7 days appears in Pi cloud_lots mirror. NOT enforceable from cloud alone — flagged as warning so the operator runs the Pi-side checker manually. Phase 4+ may push the Pi state into a small mirror table.',
    check: async (_sb) => {
  ...
  {
    name: 'stock_lots_match_4_4_9_at_food_log_write',
    severity: 'warning',
    subject_type: 'food_log',
    intent:
      'Per-row 4-4-9 invariant: |calories - (4c + 4p + 9f)| <= 10 kcal. Stricter than the daily aggregate — catches an individual log written with bad macros even if other logs in the same day cancel it out.',
    check: async (sb) => {
  ...
  {
    name: 'livetrack_session_no_stale_active',
    severity: 'warning',
    subject_type: 'livetrack_session',
    intent:
      'A LiveTrack import wizard session is live (state NOT IN closed/expired) AND > 1 hour old most likely means the browser tab was closed without finishing the wizard. The expires_at default (10 min) should have flipped it expired — if not, the expiry sweeper is broken.',
    check: async (sb) => {
  ...
  {
    name: 'coachbyte_timer_no_stale_active',
    severity: 'warning',
    subject_type: 'coachbyte_timer',
    intent:
      'A rest timer in state=running with end_time > 4h ago is a forgotten timer (the user closed the app mid-rest). Not corruption, but worth surfacing so the admin can clean it up — and a sustained backlog might indicate the auto-expire logic is broken.',
    check: async (sb) => {
  ...
  {
    name: 'product_no_macro_drift',
    severity: 'warning',
    subject_type: 'product',
    intent:
      'Products updated in the last 30 days where stored calories diverge from 4 * carbs + 4 * protein + 9 * fat by >5%. Catches a paste-error in the chef UI or an AI normalize step that wrote inconsistent macros — defends against silent macro corruption that would later be amplified through food_logs.',
    check: async (sb) => {
