...
// SetQueue uses Link from react-router-dom in empty states, so wrap renders in MemoryRouter
...
describe('SetQueue', () => {
  ...
  it('renders "No workout planned" when sets array is empty', () => {
    renderQueue(<SetQueue sets={[]} onComplete={vi.fn()} onAdHoc={vi.fn()} />);
  ...
  it('renders the next incomplete set in the NEXT IN QUEUE card', () => {
    renderQueue(<SetQueue sets={makeSets()} onComplete={vi.fn()} onAdHoc={vi.fn()} />);
  ...
  it('calls onComplete with target (pre-filled) values when Complete Set is clicked without overriding', async () => {
    ...
    renderQueue(<SetQueue sets={makeSets()} onComplete={onComplete} onAdHoc={vi.fn()} />);
  ...
  it('syncs reps/load inputs when nextSet changes (useEffect)', async () => {
    ...
    const { rerender } = renderQueue(<SetQueue sets={sets1} onComplete={vi.fn()} onAdHoc={vi.fn()} />);
    ...
    rerender(
      <MemoryRouter>
        <SetQueue sets={sets2} onComplete={vi.fn()} onAdHoc={vi.fn()} />
      </MemoryRouter>,
    );
  ...
  it('disables Complete Set and Ad-Hoc buttons when disabled prop is true', () => {
    renderQueue(<SetQueue sets={makeSets()} onComplete={vi.fn()} onAdHoc={vi.fn()} disabled />);
  ...
  it('shows "All sets completed!" when every set has completed: true', () => {
    ...
    renderQueue(<SetQueue sets={allDone} onComplete={vi.fn()} onAdHoc={vi.fn()} />);
