...
export function ScannerPage() {
  ...
  const [unit, setUnit] = useState<'servings' | 'containers'>('servings');
  ...
  const handleBarcodeSubmit = useCallback(async (barcode: string) => {
    ...
    const newItem: QueueItem = {
      ...
      mode,
      quantity: qty,
      unit: mode === 'purchase' || mode === 'shopping' ? 'containers' : unit,
      isNew: false,
      stockLevel: null,
    };
    ...
    try {
      ...
      const { data: product } = await chefbyte()
        .from('products')
        .select('product_id, name, barcode, is_placeholder, calories_per_serving, protein_per_serving, carbs_per_serving, fat_per_serving, servings_per_container')
        .eq('user_id', user.id)
        .eq('barcode', barcode)
        .single();
      ...
      if (product) {
        ...
        setQueue(prev => prev.map(item =>
          item.id === tempId
            ? { ...item, name: product.name, productId: product.product_id, status: 'success', isNew: product.is_placeholder }
            : item,
        ));
      } else {
  ...
  return (
    <ChefLayout title="Scanner">
      ...
      <div
        ...
        style={{ display: 'grid', gridTemplateColumns: '1.5fr 2.5fr', gap: '16px' }}
      >
        {/* ========================================================== */}
        {/*  LEFT COLUMN — QUEUE                                        */}
        {/* ========================================================== */}
            ...
            {filteredQueue.map(item => (
              <div
                ...
                style={{
                  padding: '8px 10px',
                  border: `2px solid ${
                    item.status === 'error' ? '#eb445a'
                    : item.status === 'pending' ? '#ffc409'
                    : item.isNew ? '#eb445a'
                    : '#2dd36f'
                  }`,
                  borderRadius: '6px',
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontWeight: 600, fontSize: '0.9em' }}>
                    {item.isNew && <span data-testid={`new-badge-${item.id}`} style={{ color: '#eb445a', marginRight: '4px' }}>[!NEW]</span>}
                    {item.name}
                  </span>
                  <button
                    data-testid={`delete-item-${item.id}`}
                    onClick={e => { e.stopPropagation(); deleteQueueItem(item.id); }}
                    style={{
                      background: 'none', border: 'none', color: '#eb445a',
        ...
        <div data-testid="keypad-panel" style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {/* Mode selector */}
          <div data-testid="mode-selector" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '8px' }}>
            {([
          ...
          {/* Active item display */}
          <div data-testid="active-item-display" style={{ padding: '8px', background: '#f4f5f8', borderRadius: '6px', textAlign: 'center', fontWeight: 600 }}>
          ...
          <div
            ...
            style={{
              padding: '12px', background: '#fff', border: '2px solid #ddd',
              borderRadius: '6px', textAlign: 'right', fontSize: '24px', fontWeight: 700,
          ...
          {mode === 'purchase' && (
            <div data-testid="nutrition-editor" style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '6px' }}>
              {([
          ...
          >
            {['7', '8', '9', '4', '5', '6', '1', '2', '3', '.', '0', '←'].map(key => (
              <button
                ...
                style={{
                  padding: '14px', border: '1px solid #ddd', borderRadius: '6px',
          ...
          {(mode === 'consume_macros' || mode === 'consume_no_macros') && (
            <IonButton
              ...
              size="small"
              fill="outline"
              onClick={() => setUnit(prev => prev === 'servings' ? 'containers' : 'servings')}
            >
              {unit === 'servings' ? 'Servings' : 'Containers'}
            </IonButton>
          )}
        </div>
      </div>
