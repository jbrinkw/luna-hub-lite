  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          'File is not a Luna Hub ChefByte backup (missing schema_version / user_id / tables).',
        );
      }
  ...
  const renderPreview = () => {
    ...
    return (
      <div
        ...
        className="mt-4 p-4 border border-border rounded-md bg-surface-sunken"
      >
        ...
        <dl className="grid grid-cols-[140px_1fr] gap-y-1 text-sm">
          ...
          >
            ...
            {parsedBackup.schema_version !== currentExpectedVersion && (
              <span className="ml-2 text-xs">
                (expected {currentExpectedVersion})
              </span>
            )}
          </dd>
          ...
          <dd className="text-text">
            {parsedBackup.generated_at
              ? new Date(parsedBackup.generated_at).toLocaleString()
              : '—'}
          </dd>
          ...
          <dd className="text-text font-mono text-xs break-all">
            ...
            {userMismatch && (
              <span
                className="ml-2 text-danger-text font-semibold text-xs"
                data-testid="preview-user-mismatch"
              >
        ...
        <ul
          ...
          className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-1 text-sm"
        >
  ...
  return (
    <div data-testid="backup-tab" className="p-5">
      ...
      <div className={cardCls} data-testid="export-section">
        ...
        <p className={descCls}>
          Saves a single JSON file containing every product, recipe, stock lot, meal-plan
          entry, food log, quick-add macro entry, shopping-list row, and macro goal tied
          to your account. Does <strong>not</strong> include Pi device state, shelf
          events, or LiveTrack scanning sessions — those are regenerated by the Pi.
        </p>
        <button
          type="button"
      ...
      <div className={cardCls} data-testid="restore-section">
        ...
        <p className={descCls}>
          <span className="text-danger-text font-semibold">Warning:</span> this will
          permanently <strong>wipe</strong> all of your current ChefByte data and
          replace it with the file's contents. There is no undo.
        </p>
        ...
        {parsedBackup && versionOk && ownerOk && (
          <label className="mt-4 flex items-start gap-2 text-sm cursor-pointer">
            ...
            <span className="text-text">
              I understand this will wipe all of my current ChefByte data and replace
              it with the contents of this backup.
            </span>
          </label>
        )}
        ...
        {parsedBackup && (
          <div className="mt-4 flex gap-2">
            ...
            {(!versionOk || !ownerOk) && (
              <span
                className="text-xs text-danger-text self-center"
