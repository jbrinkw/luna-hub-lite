...
| L11 | Invariants in code, not heads       | Design rules ("single_track must NEVER mint a lot") only exist in the user's head — anchored by the `74a0240`/`2a14d4e` revert.            | Audit every doc + `planned-work.md` + migration comment for "must/never/always" sentences. Each must trace to a pgTAP in `supabase/tests/invariants/`. Unpinned → finding. |
...
from `docs/`, `planned-work.md`, and inline migration comments. Existing
