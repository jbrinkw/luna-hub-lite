...
export const HOMEASSISTANT_turn_on: ExtensionToolDefinition = {
  name: 'HOMEASSISTANT_turn_on',
  extensionName: 'homeassistant',
  description: 'Turn on a Home Assistant device by entity ID or friendly name.',
  inputSchema: {
    type: 'object',
    properties: {
      entity_id: {
  ...
  handler: async (args, ctx) => {
    ...
    try {
      ...
      await callService(creds, domain, 'turn_on', { entity_id: entityId });

      const formatted = formatActionResult(entityId, 'turn_on', true, friendlyName);
      return toolSuccess({ formatted, entity_id: entityId, action: 'turn_on', success: true });
    } catch (e) {
