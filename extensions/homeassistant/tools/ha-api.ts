...
export function getHACredentials(ctx: ExtensionToolContext): HACredentials | null {
  ...
  // ha_url is free-form user input. Forwarding the Worker's fetch to a
  // private/loopback/metadata target would be a classic SSRF — reject any
  // such URL via the shared block list (see extensions/_lib/ssrf.ts).
  const origin = validateExternalUrl(ha_url);
  if (!origin) return null;
  return { token: ha_api_key, url: origin };
}
